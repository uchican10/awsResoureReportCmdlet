...
#>
    ...
    try {  
        $jsonOutput = aws ec2 describe-route-tables  
    ...
    # jqで必要情報を抽出  
    try {  
        $csv = $jsonOutput |jq -r '.[][]|[(.Tags[]|select(.Key=="Name")).Value // "" ,.RouteTableId]|@csv'
